...
#include "Gameplay/Calculations/SpaceCraftExecutionCalculation.h"
	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	SourceComponent->GetOwnedGameplayTags(Tags);
	/*float ReactorOn = (Tags.HasTagExact(ReactorOnTag)) ? 1 : 0;

	float PowerGenerated = ReactorOutput.GetValueAtLevel(ReactorLevel) * ReactorBoost
		* DamageEfficacy.GetValueAtLevel((MaxReactorHealth> 0) ? ReactorHealth/MaxReactorHealth : 0) * ReactorOn;
	
	float WeaponPower = PowerGenerated * WeaponDistribution;
	float EnginePower = PowerGenerated * EngineDistribution;
	float ShieldPower = PowerGenerated * ShieldDistribution;
	...
	if(ShieldLevel > 0)
	{
		float ShieldHitPoints = ShieldPower * ShieldRecharge.GetValueAtLevel(ShieldLevel) * ShieldBoost
			* DamageEfficacy.GetValueAtLevel((MaxShieldHealth> 0) ? ShieldHealth/MaxShieldHealth : 0) * Spec.Duration * ShieldOn;
	...
	float EngineOn = (Tags.HasTagExact(EngineOnTag)) ? 1 : 0;;
